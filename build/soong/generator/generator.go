...
func init() {
	android.RegisterModuleType("zenith_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = zenithExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = zenithExpandVariables(ctx, dep_file)
	...
	cmd := zenithExpandVariables(ctx, String(g.properties.Cmd))
